$(document).ready(function () {
    ...
    $.get({
      ...
      success: function (data) {
        ...
        newData.forEach((categories, i) => {
          ...
          let tableRow = $("<tr>", { id: newData.category_id });
  ...
  $("#create-new").click(function () {
    $("#modalMainLabel").html("Add Category");
  ...
  function update(index) {
    $("#modalMain").modal("show");
    $("#modalMainLabel").html("Update Category");
  ...
  function deletes(id) {
    $.post({
      url: entity + "/" + entity + "Crud.php",
      data: { category_id: id, delete: "delete" },
      success: function (data) {
        data = JSON.parse(data);
        if (data.success) {
          $("#formBodyData").trigger("reset");
  ...
  function view(index) {
    $("#modalMain").modal("show");
    $("#modalMainLabel").html("View categories");
    let models = dataArray[index];
    Object.keys(models).map((key) => {
